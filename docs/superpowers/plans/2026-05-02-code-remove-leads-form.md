...
1. Проходить `mise exec -- pnpm lint`, `mise exec -- pnpm typecheck`, `mise exec -- pnpm test` без правок `lib/database.types.ts`. Тип `leads` и `thanks_page_settings` в этом файле останутся — мы их не используем в коде, и они уйдут сами при регенерации после переезда на новую БД (часть плана C).
2. На локальном `mise exec -- pnpm dev` главная страница должна рендериться без формы и без секции «Последние заявки» в админке.
3. `pnpm build` локально не проверяем — см. «Среда выполнения» ниже.
...
**Среда выполнения (важно):**
- Все pnpm-команды в этом плане запускать через `mise exec --` — в репо `mise.toml` указывает `node = "latest"`, но shell может не быть активирован под mise. Без этого vitest падает с `ERR_REQUIRE_ESM` на старом Node.
- **`pnpm build` НЕ используется** для локальной верификации в этом плане. Причина: `next/font/google` подкачивает Montserrat/Cormorant Garamond с `fonts.googleapis.com`, что не работает без VPN. Сборка всё равно работает на стороне Onreza при деплое. Заменять Google Fonts на self-hosted — отдельный follow-up PR (см. `MEMORY.md`).
- Для локальной проверки правок полагаемся на `pnpm lint` + `pnpm typecheck` + `pnpm test` + `pnpm dev` (dev-сервер обычно работает с уже закэшированными fonts).
...
- [ ] **Step 5: Сборку НЕ прогоняем локально**

`pnpm build` пропускается в этом плане — см. «Среда выполнения» выше. На стороне Onreza при деплое сборка работает; локальная верификация ограничена lint+typecheck+test.
...
- [ ] **Step 2: Прогнать lint, typecheck, tests**
...
- [ ] **Step 2: typecheck + lint + test (без build)**
...
- [ ] **Step 3: Сборку локально не проверяем**

Корректность работы `generateStaticParams` будет видна в плане C при сборке на стороне Onreza или внутри self-hosted деплоя.
...
- [ ] **Step 1: Чистый прогон всех проверок (без build)**
...
- [ ] `mise exec -- pnpm lint && mise exec -- pnpm typecheck && mise exec -- pnpm test` — pass.
- [ ] `pnpm build` НЕ проверяется локально (Google Fonts не доступны без VPN). Build верифицируется на стороне Onreza.
